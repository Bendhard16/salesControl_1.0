...
class asControl {
   constructor() {
      ...
      this.progressControl    = function ({start, end, ins, req, rd, cont, rcp, sts}) {
         ...
         if (sts.toUpperCase() == 'DELIVERY' && req < start && new Date().getTime() >= start)            return 'past'
         else if (sts.toUpperCase() == 'DELIVERY DONE' && ins < start && new Date().getTime() >= start)  return 'past'
         ...
         if (rcp >= start && rcp <= end)                 progress = 'prepare' //console.log(progress)
         return progress
      }
