...
function checkFU () {
   ...
   if (fuPhone.value.length < 10 || fuPhone.value.length > 13 || fuPhone.value.startsWith('08') == false) status = false
   return status
}
...
function fuTaskCollect () {
   let task    = []
   document.querySelectorAll('#fu-task .task-grup').forEach(grup => {
      task.push({
...
function fuSeeDetail (IDX) {
   ...
   if (task) {
      task.data.forEach(datax => {
         createTask(fuTask, datax.note, datax.progress)
      })
   }
